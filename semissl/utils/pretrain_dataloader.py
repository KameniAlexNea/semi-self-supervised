...
def split_dataset(
    ...
    if split_strategy == "class":
        ...
        task_dataset = random_split(
            dataset, lengths, generator=torch.Generator().manual_seed(42)
        )
        if isinstance(task_idx, list):
            task_dataset = [task_dataset[i] for i in task_idx]
        else:
            task_dataset = task_dataset[task_idx]
    if split_strategy == "task":
        assert len(dataset.classes) == sum([len(t) for t in tasks])
        masks = [[(c in task) for c in dataset.targets] for task in tasks]
        indexes = [torch.tensor(mask).nonzero() for mask in masks]
        task_dataset = [Subset(dataset, index) for index in indexes]
...
def prepare_transform(dataset: str, multicrop: bool = False, **kwargs) -> Any:
    ...
    if dataset in ["cifar10", "cifar100"]:
        ...
        return (
            ...
            if not multicrop
            else MulticropImagenetTransform(**kwargs)
        )
    elif dataset == "custom":
        return (
            CustomTransform(**kwargs)
            if not multicrop
            else MulticropCustomTransform(**kwargs)
        )
