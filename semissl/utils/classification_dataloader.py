...
from semissl.utils.datasets import DomainNetDataset
from semissl.utils.pretrain_dataloader import split_dataset
...
def prepare_test_datasets(
    ...
    train_dataset = split_dataset(
        train_dataset, task_idx, num_tasks, split_strategy, tasks
    )
    val_dataset = split_dataset(val_dataset, task_idx, num_tasks, split_strategy, tasks)
...
def prepare_datasets_dsdm(
    ...
    Returns:
        Tuple[List[Dataset], List[Dataset]]: training dataset and validation dataset.
    """
    ...
    if split_strategy != "task":  # sorry for this -:) not clear but it works like
        train_datasets = [
            split_dataset(train_dataset, i, num_tasks, split_strategy, tasks)[0]
            for i in range(num_tasks)
        ]
        val_datasets = [
            split_dataset(val_dataset, i, num_tasks, split_strategy, tasks)[0]
            for i in range(num_tasks)
        ]
    else:
        train_datasets, _ = split_dataset(
            train_dataset, None, num_tasks, split_strategy, tasks
        )
        val_datasets, _ = split_dataset(
            val_dataset, None, num_tasks, split_strategy, tasks
        )
    return train_datasets, val_datasets


def prepare_data_dsdm(
    dataset: str,
    data_dir: Optional[Union[str, Path]] = None,
    train_dir: Optional[Union[str, Path]] = None,
    val_dir: Optional[Union[str, Path]] = None,
    ...
    Returns:
        Tuple[List[DataLoader], List[DataLoader]]: prepared training and validation dataloader;.
    """

    T_train, T_val = prepare_transforms(dataset)
    train_datasets, val_datasets = prepare_datasets_dsdm(
        dataset,
        T_train,
        T_val,
        data_dir=data_dir,
    ...
    train_loaders = [loaders[0] for loaders in dataloaders]
    val_loaders = [loaders[1] for loaders in dataloaders]
